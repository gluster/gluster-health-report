...
def report_non_participating_bricks(ctx):
    ...
    for session in georep_status:
        for conn in session:
            if bricks_status[conn["master_node"] + ":" + conn["master_brick"]] and \
               conn["status"] == "Offline":
                non_participating_bricks.append(conn["master_node"] + ":" + conn["master_brick"])

        if non_participating_bricks:
            ctx.notok("New bricks added to volume but not added to geo-rep. Needs action!!!")
            logging.error(ctx.lf("Bricks not added to geo-rep. Take below action.", bricks=non_participating_bricks))
            logging.info(ctx.lf("---ADD BRICK TO GEOREP-----"))
            logging.info(ctx.lf("1. gluster system:: exec gsec_create"))
            logging.info(ctx.lf("2. gluster volume geo-rep <mastervol> <slavehost>::<slavevol> create push-pem force"))
            logging.info(ctx.lf("3. gluster volume geo-rep <mastervol> <slavehost>::<slavevol> start force"))
            logging.info(ctx.lf("----------------------------"))
